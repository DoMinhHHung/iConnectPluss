...
const { createHTML } = require("docx-preview");
...
// Hàm chuyển đổi docx thành HTML
...
// Hàm tạo preview docx bằng docx-preview (giao diện phong phú hơn mammoth)
const createDocxPreview = async (buffer) => {
  try {
    const html = await createHTML(buffer, null, null, {
    ...
    console.error("Error creating DOCX preview:", error);
    // Nếu docx-preview thất bại, thử dùng mammoth
    return convertDocxToHtml(buffer);
  }
};

// Hàm chuyển đổi PDF sang HTML
const convertPdfToHtml = async (buffer) => {
  try {
    const data = await pdfParse(buffer);

    // Tạo HTML từ nội dung text được trích xuất từ PDF
    const html = `
      <!DOCTYPE html>
      <html>
        <head>
...
// Hàm tạo HTML viewer cho các file text (json, txt, markdown, v.v.)
const createTextPreview = (buffer, mimeType) => {
  try {
    let content = buffer.toString("utf-8");
    let formattedContent = content;
    let language = "";

    // Xác định loại ngôn ngữ cho highlight
...
// Hàm xem trước tệp tin dựa vào loại file
const getDocumentPreview = async (fileId) => {
  try {
    // Tìm file trong GridFS
    ...
    return new Promise((resolve, reject) => {
      ...
      downloadStream.on("end", async () => {
        try {
          // Ghép tất cả các chunk lại với nhau
          const buffer = Buffer.concat(chunks);

          // Xác định loại file và chuyển đổi thích hợp
          if (
            ...
            // Word document
            const result = await createDocxPreview(buffer);
            resolve(result);
            ...
            // Text files
            const result = createTextPreview(buffer, mimeType);
            resolve(result);
          } else {
            // Không hỗ trợ xem trước
            reject(new Error("Không hỗ trợ xem trước loại file này"));
          }
        } catch (err) {
          reject(err);
        }
...
// Endpoint cho phép xem trước file
const previewDocument = async (req, res) => {
  try {
    const fileId = req.params.fileId;

    if (!fileId) {
      return res.status(400).json({ message: "Thiếu ID file" });
    }

    const preview = await getDocumentPreview(fileId);
      ...
      <html>
        ...
        <body>
          <div class="error-container">
            <h1>Không thể xem trước tệp tin</h1>
            <p>Rất tiếc, không thể xem trước tệp tin này. Hãy tải xuống để xem.</p>
